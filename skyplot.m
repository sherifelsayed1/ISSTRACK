function hsky = skyplot(azim,elev,line_style,color)
...
q = plot(xx,yy,line_style,'color',color);
text(xx, yy, ['  ', 'ISS'], 'color', color);
...
tit=title(['< Azimuth : ' num2str(azim) '° >' '< Elevation : ' num2str(elev)  '° >'  ' <UTC Time : ' time_adapt '>']);
set(tit,'color',[1 0 0.5],'Linewidth',2);
set(tit,'Position',[5 -198]);
set(tit, 'FontSize',14)
xlim([-360 360]);
ylim([-180 180]);
